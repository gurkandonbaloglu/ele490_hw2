...
cameraman_image = Image.open('cameraman.tif').convert('L')
lisa_image = Image.open('lisa.tif').convert('L')

# Convert images to numpy arrays for further processing
cameraman_image_np = np.array(cameraman_image)
lisa_image_np = np.array(lisa_image)

output_path = 'output_images'
if not os.path.exists(output_path):
    os.makedirs(output_path)
plt.imshow(lisa_image_np, cmap='gray', vmin=0, vmax=255)
plt.title('Lisa Image')
plt.axis('off')
plt.savefig(os.path.join(output_path, 'lisa_image.png'))
plt.close()

plt.imshow(cameraman_image_np, cmap='gray', vmin=0, vmax=255)
plt.title('Cameraman Image')
plt.axis('off')
plt.savefig(os.path.join(output_path, 'cameraman_image.png'))
plt.close()

plt.hist(lisa_image_np.ravel(), bins=256, alpha=0.5, label='Lisa Original')
plt.title('Histogram of Lisa')
plt.savefig(os.path.join(output_path, 'histogram_of_lisa.png'))
plt.close()

plt.hist(cameraman_image_np.ravel(), bins=256, alpha=0.5, label='Cameraman Original')
plt.title('Histogram of Cameraman')
plt.savefig(os.path.join(output_path, 'histogram_of_cameraman.png'))
plt.close()

lisa_matched_image = exposure.match_histograms(lisa_image_np, cameraman_image_np)
...
cameraman_matched_image = exposure.match_histograms(cameraman_image_np, lisa_image_np)
...
lisa_equalized_image = cv2.equalizeHist(lisa_image_np)
...
cameraman_equalized_image = cv2.equalizeHist(cameraman_image_np)
